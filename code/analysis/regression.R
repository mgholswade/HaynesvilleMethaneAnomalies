...
# plot data (center original data for useful comparison)
df.haynesville <- df.haynesville %>%
  mutate(methane = methane - mean(methane) + mean(methane_adj))

## This plot compares the original data with the background corrected data.
## This Figure 14 in our report.
...
## Create monthly haynesville time-series plot.
...
## This is Figure 17 in our report.
...
## Get mean monthly methane values (background corrected).
vec <- tapply(df.haynesville$methane_adj, my.month(df.haynesville$time_utc), mean)
vec <- vec[-length(vec)]
shale.data$methane <- vec

## Plot time-series of monthly shale gas production in Haynesville.
## This is Figure 16 in our report.
pdf(file = "../../Figures/Regression/Shale_Gas_Prod_Line.pdf", height = 5.0, width = 7.0)
...
## Plot methane vs. shale gas production with linear fit overlaid.
## This is Figure 15 in our report.
