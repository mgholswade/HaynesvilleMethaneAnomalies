...
library(ggmap)     # For spatial plotting with maps.
...
## Loading masked data and filter only for dates beyond mid-March 2019,
## as anything before that did not have valid wind observations.
...
# Get mean methane values over time.
methaneWind <- windAdjustedMeans[, -1]
methane <- dailyMeans[, -1]
methaneWind <- apply(methaneWind, 2, mean, na.rm = TRUE)
methane <- apply(methane, 2, mean, na.rm = TRUE)

## Create plots of mean wind-adjusted methane values over all time.
# This dataframe holds wind-adjusted methane values for plotting.
df <- tibble(
  methane = methaneWind,
...
# This dataframe holds original (centered) methane values for plotting.
df2 <- tibble(
  methane2 = methane - mean(methane, na.rm = TRUE),
  longitude = rep(lon, times = RESO),
  latitude = rep(lat, each = RESO)
) %>% na.omit

## Plot of wind-adjusted methane values averaged over all time.
## This is Figure 13a in our report.
...
## Plot of original (centered) methane values averaged over all time.
## This is Figure 13b in our report.
...
#####################################################################
# The code below was exploratory and is not included in our report.
# DO NOT RUN.
#####################################################################
...
#   filter(Date >= as.Date('2019-03-01'), Date < as.Date('2020-04-01')) %>%
