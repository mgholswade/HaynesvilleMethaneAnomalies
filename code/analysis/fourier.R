...
## Filter out low-qa observations.
rm(dateTimes)
X <- X %>%
  as_tibble %>%
  filter(qa_value == 1.0)
...
# Time-series plotting bounds.
bounds <- c(1800, 1907.252)
## High QA.
# Perform fourier transform on data in our entire region.
