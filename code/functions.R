...
# Returns the week of the year, e.g. '2019-52'.
...
# Returns the month of the year, e.g. '2019-12'.
...
# This assigns sectors to each observation in a dataframe. Very useful and very
# fast. Returns updated dataframe object.
...
# This takes in a dataframe of observations, and returns a list of dataframes
# which are aggregated with respect to some unit of time (days, months, etc.)
...
# Returns a matrix of means per quadrant per period of time, e.g. means for each
# of 10,000 quadrants for each of 23 months.
...
# Performs proportion analysis and returns a matrix of size RESO x RESO, where
# each value represents the proportion of time-units (days, months, etc.) for
# which each quadrant demonstrated above-average methane levels.
...
# GGMAP wrapper for plotting proportion matrices. Returns a GGMAP object.
...
# This is a miscellaneous function called by `quilt.plot.month` which helps to
# split data up into three-day periods using modular arithmetic.
...
# Another miscellaneous function called by `quilt.plot.month` which performs
# a similar function as `setup.3day.a`.
...
# Constructs 3-day quilt plots and saves them to provided directory. No return
# value.
...
# Same function as `quilt.plot.month`, but using GGMAP. No return value. Calls
# the function just below, `ggmap.geom_tile`.
...
# This function is a wrapper for GGMAP which constructs the GGMAP object
# necessary for the 3-day plots. This function is called by
# `ggmap.plot.month`. Returns a GGMAP object.
...
# Performs wind-shift on data in provided dataframe. Returns dataframe with
# data shifted according to parameter `time_shift`.
...
# Returns the dataframe necessary to perform ARIMA analysis.
...
# Construct time-series plot for ARIMA models. Returns a GGMAP object.
...
# Performs Fourier transform on provided data. Returns two dataframes with
# Fourier transform metrics (trended and de-trended), two time-series plots
# (trended and de-trended), two frequency domain plots, two period domain plots.
