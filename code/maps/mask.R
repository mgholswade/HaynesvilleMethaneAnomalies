...
## Map with water overlaid. This is Figure 9 in our report.
...
pdf(file = '../../figures/maps/mask.pdf', height = 3.0, width = 4.125)
print(water)
dev.off()



# Here, we mask all the data near water by fitting a custom grid to the mask
# data (as it is irregular) and matching that to the same grid fit to our data.
RESO <- 2875
...
# Fit gthe rid to our data.
X <- assign.quadrants(X, latTicks, lonTicks, RESO)



# Fir the grid to the mask data.
mask <- assign.quadrants(mask, latTicks, lonTicks, RESO)
length(unique(mask$quadrant)) / RESO^2 # Should be = 1.
noData <- (1:(RESO^2))[!(1:(RESO^2) %in% unique(mask$quadrant))] # Should be empty.
...
## Determine which quadrants have water in them.
# This line of code will take a few seconds (up to 30).
...
# Save mask data to dataFullMask.Rdata, to be used by by many other files.
save(X, dateTimes, file = '../../Data/data-full/dataFullMask.Rdata')




## PROPORTION ANALYSIS ON MASKED DATA. Section 5.1 / 5.2.
RESO <- 100
latTicks <- seq(latBounds[1], latBounds[2], length = RESO + 1)
lonTicks <- seq(lonBounds[1], lonBounds[2], length = RESO + 1)
lat <- rev(rollmean(latTicks, 2))
...
# Heatmap with geo-map underlaid. This is Figure 10b.
...
################################################################
# The output of the code below was not included in our analysis,
# and none of the plots are in our report. DO NOT RUN.
################################################################
