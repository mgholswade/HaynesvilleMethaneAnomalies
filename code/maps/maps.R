...
# Map of the Haynesville-Bossier Shale.  Figure 3 in our report.
...
pdf(file = '../../figures/maps/Haynesville-Bossier.pdf', height = 3.0, width = 4.125)
print(shale.map)
dev.off()


# Sattelite view. No longer in our report.
...
pdf(file = '../../figures/maps/satellite.pdf', height = 3.0, width = 4.125)
print(sat.map)
dev.off()
...
## 10,000-sector map. Figure 7 in our report.
RESO <- 100
latTicks <- seq(latBounds[1], latBounds[2], length = RESO + 1)[-1] %>% .[-RESO]
lonTicks <- seq(lonBounds[1], lonBounds[2], length = RESO + 1)[-1] %>% .[-RESO]
pdf(file = '../../figures/maps/highqmap.pdf', height = 3.0, width = 4.125)
